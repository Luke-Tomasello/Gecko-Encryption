...
static void AddRoundKey(uint8_t round, state_t* state, uint8_t* RoundKey)
{
	...
	for (i = 0; i < 4; ++i)
	{
		for (j = 0; j < 4; ++j)
		{
#ifndef PUMPKIN_Recorder
			uint8_t temp = RoundKey[(round * Nb * 4) + (i * Nb) + j];
			PUMPKIN_Recorder(temp,0);
			(*state)[i][j] ^= temp;
#else
	...
	for (i = 0, bi = AES_BLOCKLEN; i < length; ++i, ++bi)
	{
		if (bi == AES_BLOCKLEN) /* we need to regen xor compliment in buffer */
		{
			...
			for (bi = (AES_BLOCKLEN - 1); bi >= 0; --bi)
			{
				...
				{
					ctx->Iv[bi] = 0;
					continue;			//lt: 255 was 'used' the last pass
				}
				ctx->Iv[bi] += 1;
				break;					//lt: aes only increments one byte, then breaks
			}
			bi = 0;
		}

		//lt: never a zero mask? But why?
