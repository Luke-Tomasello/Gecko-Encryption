...
void gt_HelloWorldOFB(int test)
{
	...
	printf("TEST%d: Gecko Hello World! Test Setting up...\n", test);
	{
		...
		state.encryption_mode = GKO_mode_ofb;								// Set OFB mode
...
// special secret data for the OFB Sender/Receiver demonstration 
//	This is only a demonstration of OFB functionality, not how keys are shared across apps/services, etc.
//	you will likely have your own mechanism. We perform the key exchange here only to demonstrate the separation of 
//	the Sender and Receiver
#if defined(_GKO256)
const uint8_t GKO_OFB_TEST_SECRET_key[GKO_KEY_NELTS] = {	0xd2, 0xa6, 0xab, 0xf7, 0x15, 0xca, 0x71, 0xbe, 0x2b, 0x73, 0xae, 0xf0, 0x85, 0x7d, 0x77, 0x81, 
								0x1f, 0x35, 0x2c, 0x07, 0x3b, 0x61, 0x08, 0xd7, 0x2d, 0x98, 0x10, 0xa3, 0x09, 0x14, 0xdf, 0xf4 };
#elif defined(_GKO192)
// we will add 8 bytes here for padding since a 24 bit key is not GKO_BLOCK_SIZE % 16
//	we will pad below
const uint8_t GKO_OFB_TEST_SECRET_key[GKO_KEY_NELTS] = {	0x2d, 0x98, 0x10, 0xa3, 0xda, 0x0e, 0x64, 0x52, 0xc8, 0x10, 0xf3, 0x2b, 0x80, 0x90, 0x79, 0xe5,
								0x62, 0xf8, 0xea, 0xd2, 0x52, 0x2c, 0x6b, 0x7b }; 
#elif defined(_GKO128)
const uint8_t GKO_OFB_TEST_SECRET_key[GKO_KEY_NELTS] = { 0x60, 0x3d, 0xeb, 0x10, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c };
#endif

const uint8_t GKO_OFB_TEST_SECRET_iv[GKO_IV_BUFFER_SIZE] = 
{ 0x62, 0xf8, 0xea, 0xd2, 0x52, 0x2c, 0x6b, 0x7b, 0xc8, 0x10, 0xf3, 0x2b, 0x80, 0x90, 0x79, 0xe5 };

#define OFB_TEST_MAX_KEY 32		// we need to pad for 24byte keys to 32 bytes
#define OFB_TEST_SET_KEYIV 0
#define OFB_TEST_INIT 1
#define OFB_TEST_DATA 2
#define OFB_TEST_DONE 3
static void gt_CallHelloWorldOFBReceiver(int mode, uint8_t _buff1[OFB_TEST_MAX_KEY], uint8_t _buff2[GKO_IV_BUFFER_SIZE])
{
	// static to retain state across calls
	static GKO_state_t state;
	uint16_t new_size;
	// the Sender will send us the key and IV
	static uint8_t GKO_OFB_TEST_SESSION_key[GKO_KEY_NELTS];
	// OFB needs an IV. The GKO_OFB_TEST_SESSION_iv is used to store this data, that's why it needs to be static, malloc'ed, etc.
	static uint8_t GKO_OFB_TEST_SESSION_iv[GKO_IV_BUFFER_SIZE];
	uint8_t buff1[OFB_TEST_MAX_KEY], buff2[GKO_IV_BUFFER_SIZE];

	switch (mode)
	{
	case OFB_TEST_SET_KEYIV:
		if (GKO_KEY_NELTS == 24)	// special case for 192 bit keys
			memcpy(buff1, _buff1,OFB_TEST_MAX_KEY);
		else
		...
		state.encryption_mode = GKO_mode_ofb;							// Set OFB mode
		GKO_InitState(&state, GKO_OFB_TEST_SECRET_key, GKO_OFB_TEST_SECRET_iv);											
		GKO_Decrypt(&state, buff1, GKO_KEY_NELTS == 24 ? OFB_TEST_MAX_KEY : GKO_KEY_NELTS);
		if (GKO_KEY_NELTS == 24)	
		{	// special case for 192 bit keys, unpack the 24 bytes from the 32 bytes
			new_size = GUTL_UnPadBlockRaw(buff1, OFB_TEST_MAX_KEY);
			assert(new_size == GKO_KEY_NELTS);
		}
		memcpy(GKO_OFB_TEST_SESSION_key, buff1, GKO_KEY_NELTS);
		GKO_Decrypt(&state, buff2, GKO_IV_BUFFER_SIZE);
		memcpy(GKO_OFB_TEST_SESSION_iv, buff2, GKO_IV_BUFFER_SIZE);
		break;
	case OFB_TEST_INIT:
		state.encryption_mode = GKO_mode_ofb;							// Set OFB mode
		GKO_InitState(&state, GKO_OFB_TEST_SESSION_key, GKO_OFB_TEST_SESSION_iv);											
		break;
	case OFB_TEST_DATA:
	{
		uint16_t cx;
...
void gt_HelloWorldOFBSender(int test)
{
	...
	//	we will need to pad it out to 32 bytes (OFB_TEST_MAX_KEY)
	static uint8_t GKO_OFB_TEST_SESSION_key[OFB_TEST_MAX_KEY];	
	static uint8_t GKO_OFB_TEST_SESSION_iv[GKO_IV_BUFFER_SIZE];
	uint8_t session_key_buf[OFB_TEST_MAX_KEY], session_iv_buf[GKO_IV_BUFFER_SIZE];
	char working_buffer[sizeof(Perrault) + 16 + 1];	

	printf("TEST%d: Gecko OFB 'Send Receive' Test Setting up...\n", test);
	working_buffer[sizeof(Perrault) + 16] = 0x88;							// sentinel (sanity check)

	// first we will generate an IV for this OFB session.
	for (ix = 0; ix < GKO_IV_BUFFER_SIZE; ix++)
		GKO_OFB_TEST_SESSION_iv[ix] = (uint8_t)rand() % 256;

	// now we will generate a session key for this OFB session.
	for (ix = 0; ix < GKO_KEY_NELTS; ix++)
		GKO_OFB_TEST_SESSION_key[ix] = (uint8_t)rand() % 256;

	// make copies for our Receiver
	memcpy(session_key_buf,GKO_OFB_TEST_SESSION_key,GKO_KEY_NELTS);
	memcpy(session_iv_buf, GKO_OFB_TEST_SESSION_iv, GKO_IV_BUFFER_SIZE);
	...
	gt_CallHelloWorldOFBReceiver(OFB_TEST_SET_KEYIV, session_key_buf, session_iv_buf);

	// tell our Receiver to also init
	gt_CallHelloWorldOFBReceiver(OFB_TEST_INIT, NULL, NULL);
	...
	for (ix = 0; ix < new_size; ix += GKO_BLOCK_SIZE)
	{
		...
		gt_CallHelloWorldOFBReceiver(OFB_TEST_DATA, &working_buffer[ix], NULL);
	}

	// tell our Receiver we are done
	gt_CallHelloWorldOFBReceiver(OFB_TEST_DONE, NULL, NULL);

	assert(working_buffer[sizeof(Perrault) + 16] == (char)0x88);			// just sanity checking

	printf("TEST%d: Gecko OFB 'Send Receive' Test complete.\n", test);
	printf("**********************************\n\n");
}
